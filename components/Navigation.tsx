...
export function SidebarNav({ active, onChange }: { active: NavId; onChange: (id: NavId) => void }) {
  return (
    <aside className="fixed left-0 top-0 hidden h-screen w-24 flex-col items-center border-r border-line bg-black/20 px-3 py-6 backdrop-blur-xl lg:flex">
...
export function BottomNav({ active, onChange }: { active: NavId; onChange: (id: NavId) => void }) {
  return (
    <nav className="fixed left-4 right-4 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-40 grid grid-cols-5 gap-1 rounded-3xl border border-line bg-[#090d18]/92 p-2 shadow-soft backdrop-blur-xl lg:hidden">
      {navItems.map((item) => (
        <button
          key={item.id}
          className={cn(
